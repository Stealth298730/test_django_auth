...
def delete_contact(request,id):
    ...
    contact.delete()
    messages.add_message(request=request,level=messages.SUCCESS,messages=f"Контакт'{contact}' успішно видалено")
    return redirect("get_contacts")
...
def edit_contact(request,id):
    ...
    if request.POST and form.changed_data:
        form.save()
        messages.add_message(request=request,message="Дані успішно оновлено",level=messages.SUCCESS)
        return redirect("get_contacts")
