...
class Planner(models.Model):
    ...
    link = models.URLField(max_length=300,null=True,blank=True,default=True)
    accepted = models.BooleanField(default=None,null=True,blank=True)

    def __str__(self):
        return f"Зустріч '{self.title}' з {self.contact.first_name} {self.contact.last_name}:{self.date} o {self.time} "